...
// A llama.cpp *backend* sampler: top-k plus a full-vocabulary log
// normalizer, emitted as graph nodes so they execute on whatever device the
// model runs on instead of on the host.
//
...
// that decides the tree's shape. The graph selects first and normalizes
// second (softmax is monotonic, so the survivors are the same either way),
// which keeps the arithmetic identical to the Python reference's
// log_softmax(...).topk(...) while avoiding a vocabulary-wide ggml_soft_max
// that the Hexagon backend refuses above 131072 columns.
//
